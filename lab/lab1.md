...
quest 8:
## Question 8
I first switched to the older Git commit where 'data.dvc' represented the previous version of the dataset.
I ran:
```bash
git checkout 0211c06
dvc checkout
After running these commands, the food11_processed and food11_processed_mini folders disappeared and only the raw Food-11 dataset remained.
This happened because Git restored the older version of the data.dvc pointer file, and DVC restored the actual data corresponding to that version.
I then returned to the latest version using:
git checkout main
dvc checkout
The food11_processed and food11_processed_mini folders were restored. This shows how Git is used to version the DVC pointer while DVC manages the actual dataset.
````
